        ...
        if bx > -ATTACK_MARGIN: # little margin
            ...
            if by > up_pad or by < down_pad or bx > right_pad or bx < left_pad:
                ...
                if ((static_theta >= math.pi/2 or static_theta <= -math.pi/2) and (y >= by-0.07 and y <= by+0.07) and x > bx) and (dist_to_ball <=0.4): 
                    #printWrapper("Corner : " + str(id)+ " - inner, close")
                    if y<by:
                        wheel_velos = self.motors[id].move_to_target(my_posture, [x,y-1])
                    else:
                        wheel_velos = self.motors[id].move_to_target(my_posture, [x,y+1])
               
                # ball is inner area&far, now
                elif ((static_theta >= math.pi/2 or static_theta <= -math.pi/2) and (y >= by-0.07 and y <= by+0.07) and x > bx):
                    #printWrapper("Corner : " + str(id)+ " - inner, far")
                ...
                #        printWrapper(str(id)+": INNER-INNER CORNER")
                #        if y<by:
                #            wheel_velos = self.motors[id].move_to_target(my_posture, [x,y-1])
                #        else:
                #            wheel_velos = self.motors[id].move_to_target(my_posture, [x,y+1])
                ...
                #    else:
                #        printWrapper(str(id)+": INNER-OUTER CORNER")
                ...
                # ball is in outer area, so PUSH to goalpstn
                    ...
                    # ball is at other corner
                        ...
                        #printWrapper("Corner : " + str(id)+ " - outer")
                        NUM_OF_PREDICTED_FRAMES = 1
                        target = [cur_ball[0] + cur_trans[0]*NUM_OF_PREDICTED_FRAMES,
                                  cur_ball[1] + cur_trans[1]*NUM_OF_PREDICTED_FRAMES]
                        wheel_velos = self.motors[id].move_to_target(my_posture, target)
            ...
            # Inside ground
                ...
                # ball is inner area now, and close
                if ((static_theta >= math.pi/2 or static_theta <= -math.pi/2) and (y >= by-0.07 and y <= by+0.07) and x > bx) and (dist_to_ball <=0.4): 
                    #printWrapper("Ground : " + str(id)+ " - inner, close")
                    if y<by:
                        wheel_velos = self.motors[id].move_to_target(my_posture, [x,y-1])
                    else:
                        wheel_velos = self.motors[id].move_to_target(my_posture, [x,y+1])
                ...
                # ball is inner area&far, now
                    ...
                    #printWrapper("Ground : " + str(id)+ " - inner, far")
                    NUM_OF_PREDICTED_FRAMES = 2
                    target = [cur_ball[0] + cur_trans[0]*NUM_OF_PREDICTED_FRAMES,
                              cur_ball[1] + cur_trans[1]*NUM_OF_PREDICTED_FRAMES]
                    wheel_velos = self.motors[id].move_to_target(my_posture, target, damping=0)
                ...
                # ball is in outer area, so PUSH to goalpstn
                    ...
                    if (dist_to_ball <= 0.4):
                        #printWrapper("Ground : " + str(id)+ " - outer, close")
                        cur_trans = self.data_proc.get_cur_ball_transition()
                        goal_pstn = [1.9, 0]
                        tar_posture = self.cal.compute_desired_posture(cur_ball, cur_trans, goal_pstn)
                        tar_posture = [tar_posture[0]-0.08,tar_posture[1]-0.08,0]
                        ...
                        #printWrapper("Ground : " + str(id)+ " - outer, far")
                        NUM_OF_PREDICTED_FRAMES = 1
                        target = [cur_ball[0] + cur_trans[0]*NUM_OF_PREDICTED_FRAMES,
                                  cur_ball[1] + cur_trans[1]*NUM_OF_PREDICTED_FRAMES]
                        wheel_velos = self.motors[id].move_to_target(my_posture, target)
...
#            elif (bx > 0.5) and (by>=-1.0 and by<=1.0):
                printWrapper("IN")
                backup_x = 1.3
                backup_y = GOAL_WIDTH
