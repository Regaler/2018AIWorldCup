...
class Strategy(object):
    ...
    def attack(self, id, default_posture):
        ...
        printWrapper("transition: " + str(cur_trans[0]))
        # ATTACK state
        if cur_ball[0] > -MARGIN: # little margin
            ...
            if cur_ball[1] > up_pad or cur_ball[1] < down_pad or cur_ball[0] > right_pad or cur_ball[0] < left_pad:
                if my_posture[1] >= 0: # It's blocking right top corner
                    if (my_posture[0] <= FIELD_LENGTH/2 and my_posture[0] >= FIELD_LENGTH/2-BALL_RADIUS*2-GOAL_DEPTH) and (self.cal.r2d(my_posture[2]) >= 80 and self.cal.r2d(my_posture[2] <= 100)):
                        wheel_velos = [-1.8, -1.5]
                else: # It's blocking left bottom corner
                    if (my_posture[0] <= FIELD_LENGTH/2 and my_posture[0] >= FIELD_LENGTH/2-BALL_RADIUS*2-GOAL_DEPTH) and (self.cal.r2d(my_posture[2]) >= 80+180 and self.cal.r2d(my_posture[2] <= 100+180)):
                        wheel_velos = [-1.8, -1.5]
                wheel_velos = self.motors[id].three_phase_move_to_target(my_posture, cur_ball, damping=0)
                return wheel_velos
            # Inside ground
            ox = 0.2
            oy = 0.2
            ...
            if id == self.upper_attacker:
                if cur_trans[0] <= -0.015:
                    ...
                    if cur_ball[0] <= -FIELD_LENGTH/2+PENALTY_AREA_DEPTH/2+GOAL_DEPTH:
                        wheel_velos = self.motors[id].three_phase_move_to_target(my_posture, [-FIELD_LENGTH/2, PENALTY_AREA_WIDTH/2+0.01, math.pi/2])
                    else:
